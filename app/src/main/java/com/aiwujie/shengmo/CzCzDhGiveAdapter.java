 ...
 */
public class CzCzDhGiveAdapter extends BaseAdapter {
    ...
    public CzCzDhGiveAdapter(Context context, List<ConsumptionGiveData.DataBean> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.item_czczdh_recorder, null);
        ...
        if(data.getAmount().equals("0")&&data.getBeans().equals("0")){
            ...
            switch (data.getState()) {
                case "1":
                    ...
                    break;
                case "2":
                    holder.itemRecorderRmb.setText("赠" + data.getNickname() + vips[Integer.parseInt(data.getType()) - 1] + "[" + data.getAmount() + "元]");
                    break;
            }
        }else{
            holder.itemRecorderModou.setText("-"+data.getBeans()+"礼物魔豆");
            switch (data.getState()) {
                case "1":
                    holder.itemRecorderRmb.setText("赠" + data.getNickname() + "[" + titles[Integer.parseInt(data.getType()) - 10] + "]x" + data.getNum() + "[" + data.getAmount() + "元]");
                    break;
                case "2":
                    holder.itemRecorderRmb.setText("赠" + data.getNickname() + vips[Integer.parseInt(data.getType()) - 1] + "[" + data.getAmount() + "元]");
                    break;
