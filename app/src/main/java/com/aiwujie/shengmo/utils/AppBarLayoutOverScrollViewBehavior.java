...
public class AppBarLayoutOverScrollViewBehavior extends AppBarLayout.Behavior {
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, AppBarLayout abl, int layoutDirection) {
        ...
        if (mToolBar == null) {
            ...
            //mToolBar = parent.findViewById();
        }
        if (middleLayout == null) {
           // middleLayout = (ViewGroup) parent.findViewWithTag(TAG_MIDDLE);
            middleLayout = (ViewGroup) parent.findViewById(R.id.collapsingToolbarLayout_user_info);
        }
        // 需要在调用过super.onLayoutChild()方法之后获取
        if (mTargetView == null) {
            mTargetView = parent.findViewById(R.id.view_user_info_cover_bottom);
            if (mTargetView != null) {
                initial(abl);
            }
        }
