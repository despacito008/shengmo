...
import com.aiwujie.shengmo.adapter.ConsumptionGiveAdapter;
 ...
 */
public class FragmentPresentExchangeGive extends Fragment implements PullToRefreshBase.OnRefreshListener2 {
    ...
    private int page = 0;
    List<ConsumptionGiveData.DataBean> bills = new ArrayList<>();
    Handler handler = new Handler();
    private ConsumptionGiveAdapter listviewAdapter;
...
//        setListener();
    ...
    private void getBills() {
        ...
        manager.post(HttpUrl.GetGivePsRerond, map, new IRequestCallback() {
            @Override
            public void onSuccess(final String response) {
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ConsumptionGiveData listData = new Gson().fromJson(response, ConsumptionGiveData.class);
                            if (listData.getData().size() == 0) {
                                ...
                                if (page == 0) {
                                    ...
                                    try {
                                        listviewAdapter = new ConsumptionGiveAdapter(getActivity(), bills);
