...
import com.aiwujie.shengmo.adapter.RecorderAdapter;
 ...
 */
public class FragmentCzCzptionGive extends Fragment implements PullToRefreshBase.OnRefreshListener2 {
    ...
    private int page = 0;
    List<BillData.DataBean> bills = new ArrayList<>();
    Handler handler = new Handler();
    private RecorderAdapter listviewAdapter;
...
//        setListener();
    ...
    private void getBills() {
        ...
        manager.post(HttpUrl.GetWalletRecord, map, new IRequestCallback() {
            @Override
            public void onSuccess(final String response) {
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            BillData listData = new Gson().fromJson(response, BillData.class);
                            if (listData.getData().size() == 0) {
                                ...
                                if (page == 0) {
                                    ...
                                    try {
                                        listviewAdapter = new RecorderAdapter(getActivity(), bills);
