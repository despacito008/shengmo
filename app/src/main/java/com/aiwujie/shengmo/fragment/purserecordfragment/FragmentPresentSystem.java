...
import com.aiwujie.shengmo.adapter.PresentReceiveSystemAdapter;
...
public class FragmentPresentSystem extends Fragment implements PullToRefreshBase.OnRefreshListener2{
    ...
    private int page = 0;
    List<PresentSystemData.DataBean> bills = new ArrayList<>();
    Handler handler = new Handler();
    private PresentReceiveSystemAdapter listviewAdapter;
    ...
    private void getBills() {
        ...
        manager.post(HttpUrl.GetBasicGiveRecord, map, new IRequestCallback() {
            @Override
            public void onSuccess(final String response) {
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            PresentSystemData listData = new Gson().fromJson(response, PresentSystemData.class);
                            if (listData.getData().size() == 0) {
                                ...
                                if (page == 0) {
                                    ...
                                    try {
                                        listviewAdapter = new PresentReceiveSystemAdapter(getActivity(), bills);
