...
import com.aiwujie.shengmo.adapter.StampReceiveAdapter;
 ...
 */
public class FragmentStampReceive extends Fragment implements PullToRefreshBase.OnRefreshListener2 {
    ...
    private int page = 0;
    List<StampReceiveData.DataBean> bills = new ArrayList<>();
    Handler handler = new Handler();
    private StampReceiveAdapter listviewAdapter;
...
//        setListener();
    ...
    private void getBills() {
        ...
        manager.post(HttpUrl.GetBasicStampGiveRs, map, new IRequestCallback() {
            @Override
            public void onSuccess(final String response) {
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            StampReceiveData listData = new Gson().fromJson(response, StampReceiveData.class);
                            if (listData.getData().size() == 0) {
                                ...
                                if (page == 0) {
                                    ...
                                    try {
                                        listviewAdapter = new StampReceiveAdapter(getActivity(), bills);
