...
import com.aiwujie.shengmo.adapter.PresentExchangeWithdrawalsAdapter;
 ...
 */
public class FragmentPresentExchangeWithdrawals extends Fragment implements PullToRefreshBase.OnRefreshListener2 {
    ...
    private int page = 0;
    List<PresentExchangeWithdrawalsData.DataBean> bills = new ArrayList<>();
    Handler handler = new Handler();
    private PresentExchangeWithdrawalsAdapter listviewAdapter;
...
//        setListener();
    ...
    private void getBills() {getBills2();}
    private void getBills2() {
        Map<String, String> map = new HashMap<>();
        map.put("uid", MyApp.uid);
        map.put("page", page + "");
        IRequestManager manager = RequestFactory.getRequestManager();
        manager.post(HttpUrl.GetWithdrawedRecord, map, new IRequestCallback() {
            @Override
            public void onSuccess(final String response) {
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            PresentExchangeWithdrawalsData listData = new Gson().fromJson(response, PresentExchangeWithdrawalsData.class);
                            if (listData.getData().size() == 0) {
                                ...
                                if (page == 0) {
                                    ...
                                    try {
                                        listviewAdapter = new PresentExchangeWithdrawalsAdapter(getActivity(), bills);
