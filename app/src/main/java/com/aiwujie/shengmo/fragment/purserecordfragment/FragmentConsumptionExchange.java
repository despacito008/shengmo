...
import com.aiwujie.shengmo.adapter.ConsumptionExchangeAdapter;
 ...
 */
public class FragmentConsumptionExchange extends Fragment implements PullToRefreshBase.OnRefreshListener2 {
    ...
    private int page = 0;
    List<ConsumptionExchangeData.DataBean> bills = new ArrayList<>();
    Handler handler = new Handler();
    private ConsumptionExchangeAdapter listviewAdapter;
...
//        setListener();
    ...
    private void getBills() {
        ...
        manager.post(HttpUrl.GetExchangeRecord, map, new IRequestCallback() {
            @Override
            public void onSuccess(final String response) {
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ConsumptionExchangeData listData = new Gson().fromJson(response, ConsumptionExchangeData.class);
                            if (listData.getData().size() == 0) {
                                ...
                                if (page == 0) {
                                    ...
                                    try {
                                        listviewAdapter = new ConsumptionExchangeAdapter(getActivity(), bills);
