...
import com.aiwujie.shengmo.adapter.EjectionBuyAdapter;
 ...
 */
public class FragmentEjectionBuy extends Fragment implements PullToRefreshBase.OnRefreshListener2 {
    ...
    private int page = 0;
    List<StampBuyData.DataBean> bills = new ArrayList<>();
    Handler handler = new Handler();
    private EjectionBuyAdapter listviewAdapter;
...
//        setListener();
    ...
    private void getBills() {
        ...
        manager.post(HttpUrl.getTopcardPaymentRs, map, new IRequestCallback() {
            @Override
            public void onSuccess(final String response) {
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            StampBuyData listData = new Gson().fromJson(response, StampBuyData.class);
                            if (listData.getData().size() == 0) {
                                ...
                                if (page == 0) {
                                    ...
                                    try {
                                        listviewAdapter = new EjectionBuyAdapter(getActivity(), bills);
