...
import com.aiwujie.shengmo.kt.adapter.LivePlayBackRankAdapter
import com.aiwujie.shengmo.kt.adapter.LiveRedEnvelopesRankAdapter
 ...
 * 直播排行榜 - 回放榜
 */
class LivePlayBackRankFragment : LazyFragment() {
    ...
    companion object {
        ...
        fun newInstance(aType: Int) = LivePlayBackRankFragment().apply {
    ...
    private fun getRankingData() {
        HttpHelper.getInstance().getLivePlayBackRankList(type, page, object : HttpCodeListener {
                    ...
                    liveRankBean?.data?.let {
                        when (page) {
                            0 -> {
                                ...
                                rankList.clear()
                                rankList.addAll(it)
                                adapter = LiveAnchorRankAdapter(activity, rankList, type,2)
