...
import com.aiwujie.shengmo.adapter.PublicScreenPushTopAdapter
 ...
 * 大喇叭 - 动态推顶
 */
class PublicScreenPushTopFragment : LazyFragment() {
    var page = 0
    var publicScreenAdapter: PublicScreenPushTopAdapter? = null
    lateinit var noticeList: ArrayList<NoticePresentData.DataBean>
    override fun loadData() {
        HttpHelper.getInstance().getNoticeTopCardData( page, object : HttpCodeMsgListener {
                    ...
                    tempData?.data?.let {
                        when (page) {
                            0 -> {
                                ...
                                publicScreenAdapter = PublicScreenPushTopAdapter(activity, noticeList,this@PublicScreenPushTopFragment)
                                rvPublicScreen.adapter = publicScreenAdapter
                                rvPublicScreen.layoutManager = LinearLayoutManager(activity)
                                publicScreenAdapter?.setOnNoticeItemClick(object :PublicScreenPushTopAdapter.OnNoticeItemClick {
