 ...
 * 消息 - 评论 - 推顶过我的
 */
class PushTopUsActivity : NormalListActivity() {
                ...
                tempData?.data?.let {
                    ...
                    when (page) {
                        0 -> {
                            ...
                            pushTopAdapter = PushTopUsAdapter(this@PushTopUsActivity,pushTopList)
                            rvList.adapter = pushTopAdapter
                            rvList.layoutManager = LinearLayoutManager(this@PushTopUsActivity)
                            pushTopAdapter?.setOnSimpleItemListener {
                                var intent = Intent(this@PushTopUsActivity,NewDynamicDetailActivity::class.java)
