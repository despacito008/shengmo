...
class HighEndAuthWealthFragment : LazyFragment() {
    ...
    private fun initRecycleViewAdapter() {
        mHighAuthAdapter = HighAuthAdapter(context, mList, "1")
        with(recycleView) {
    ...
    private fun getAuthInfo() {
        HttpHelper.getInstance().getHighAuthInfo("1", object : HttpCodeMsgListener {
