...
import com.aiwujie.shengmo.adapter.PushTopUseAdapter
 ...
 * 推顶卡明细 - 使用记录
 */
class PushTopUseFragment: NormalListFragment() {
    ...
    var page = 0
    var pushTopAdapter: PushTopUseAdapter? = null
    lateinit var pushTopList:ArrayList<DynamicDetailBean.DataBean>
    fun getPushTopData() {
        HttpHelper.getInstance().getPushTopUseData(page,object : HttpCodeMsgListener {
                    ...
                    tempData?.data?.let {
                        ...
                        when (page) {
                            0 -> {
                                ...
                                pushTopAdapter = PushTopUseAdapter(activity,pushTopList)
