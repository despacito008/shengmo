...
//礼物弹窗
class GiftPanelPop3(context: Context?) : BasePopupWindow(context) {
    var type: Int = 0
   ...
   // lateinit var vpGift: ViewPager
    ...
    private val tvFans:TextView by lazy { findViewById<TextView>(R.id.tv_pop_gift_panel_fans) }
    ...
    override fun onCreateContentView(): View {
        return createPopupById(R.layout.app_pop_gift_panel3)
    }

    lateinit var vpGift:ViewPager
    lateinit var idGift:CircleIndicator
    lateinit var vpFans:ViewPager
    lateinit var idFans:CircleIndicator
    lateinit var vpKnapsack:ViewPager
    ...
    fun initViewPager() {
        ...
        //粉丝团
        val fansView = LayoutInflater.from(context).inflate(R.layout.app_layout_panel_gift, vpPanel, false)
        vpFans = fansView.findViewById<ViewPager>(R.id.vp_layout_gift)
        idFans = fansView.findViewById<CircleIndicator>(R.id.indicator_layout_gift)
        ...
        HttpHelper.getInstance().getGiftList(object : HttpCodeListener {
            override fun onSuccess(data: String?) {
                ...
                giftData?.apply {
                    ...
                    val fansGoodsList = this.data.fanclub_gift
                    val mFansPagerList = ArrayList<View>()
                    fansGoodsList?.let {
                        for (i in 0..fansGoodsList.size / 9) {
                            val tempList = fansGoodsList.subList(i * 9, min(i * 9 + 9, fansGoodsList.size))
                            if (tempList.size > 0) {
                                val rvPanel: RecyclerView = inflater.inflate(R.layout.app_view_gift_panel, vpGift, false) as RecyclerView
                                val giftPanelAdapter = GiftPanelAdapter(context, tempList)
                                val layoutManager = GridLayoutManager(context, 3)
                                ...
                                mFansPagerList.add(rvPanel)
                            }
                        }
                        vpFans.adapter = NormalViewPagerAdapter(mFansPagerList)
                        //indicator.setViewPager(vpGift)
                        idFans.setViewPager(vpFans)
                    }
