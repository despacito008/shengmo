 ...
 * @Package: com.aiwujie.shengmo.kt.ui.activity
 * @ClassName: SelfieAuthActivity
 * @Author: xmf
 * @CreateDate: 2022/4/8 15:42
 * @Description: 自拍认证
 */
class SelfieAuthActivity:BaseActivity() {
    // 标题栏
    ...
    private lateinit var tvAuthOpen: TextView
    private lateinit var tvAuthClose: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ivAuthPic = findViewById(R.id.mPhoto_auth_pic)
        tvAuthOpen = findViewById(R.id.mPhoto_auth_btn02)
        tvAuthClose = findViewById(R.id.mPhoto_auth_btn01)
        tvAuthCommit = findViewById(R.id.mPhoto_auth_tijiao)

        ivAuthPic.setOnClickListener {
            requestCameraPermission()
        }
        tvAuthOpen.clickDelay {
            if (!mOpenPic) {
                mOpenPic = true
                tvAuthOpen.isSelected = true
                tvAuthClose.isSelected = false
                setSelfieOpenState()
            }
        }
        tvAuthClose.clickDelay {
            if (mOpenPic) {
                mOpenPic = false
                tvAuthOpen.isSelected = false
                tvAuthClose.isSelected = true
                setSelfieOpenState()
            }
        }
        tvAuthCommit.clickDelay {
            commitSelfieAuth()
        }
        getSelfieState()
    }
    ...
    private fun getSelfieState() {
        HttpHelper.getInstance().getSelfieState(object :HttpCodeMsgListener {
            override fun onSuccess(data: String?, msg: String?) {
                ...
                result?.data?.run {
                    if (state != "999") {
                        ImageLoader.loadImageWithoutError(this@SelfieAuthActivity, card_face, ivAuthPic)
                    }
                    mOpenPic = status == "1"
                    if (mOpenPic) {
                        tvAuthOpen.isSelected = true
    ...
    private fun setSelfieOpenState() {
        HttpHelper.getInstance().setSelfieOpenState(mOpenPic,object :HttpCodeMsgListener {
            override fun onSuccess(data: String?, msg: String?) {
                msg?.showToast()
            }
    ...
    private fun showCameraTipDialog() {
        val dialog = with(AlertDialog.Builder(this)) {
            setTitle("提示")
            setMessage("该功能需要使用 相机 和 存储权限，授予后才能正常使用")
            setPositiveButton("去授予") { dialog, _ ->
                dialog.dismiss()
                XXPermissions.startPermissionActivity(this@SelfieAuthActivity, Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE)
            }
    ...
    private fun uploadImage(uri: Uri) {
        ...
        HttpHelper.getInstance().uploadImage(path, object : HttpCodeMsgListener {
            override fun onSuccess(data: String?, msg: String?) {
                ...
                beanIcon?.data?.run {
                    val imgPre = SharedPreferencesUtils.getParam(this@SelfieAuthActivity, SpKey.IMAGE_HOST, "") as String
                    mPicUrl = imgPre + this
                    ImageLoader.loadImage(this@SelfieAuthActivity, mPicUrl, ivAuthPic)
                }
            }
    ...
    private fun commitSelfieAuth() {
        if (mPicUrl.isEmpty()) {
            "请上传自拍照片".showToast()
            return
        }
        HttpHelper.getInstance().commitSelfieAuth(mPicUrl,object :HttpCodeMsgListener{
            override fun onSuccess(data: String?, msg: String?) {
                msg?.showToast()
                finish()
            }
