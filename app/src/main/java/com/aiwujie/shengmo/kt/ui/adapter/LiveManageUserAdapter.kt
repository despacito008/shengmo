...
class LiveManageUserAdapter(private val context: Context, private var onLineUserList: List<ManagerList.DataBean>) : RecyclerView.Adapter<LiveManageUserAdapter.LiveOnlineUserHolder>() {
    ...
    inner class LiveOnlineUserHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private var tvItemCity: TextView = itemView.findViewById(R.id.tv_item_live_online_user_city)
        var isSafeClick = true
        fun display(index: Int) {
            onLineUserList[index].let {
                ...
                if ("1" == it.location_city_switch) {
                    tvItemCity.text = "未知城市"
                } else {
                    tvItemCity.text = it.city
                }
            ...
            tvItemRemove.setOnClickListener {
                ...
                itemView.postDelayed({ isSafeClick = true }, 1000)
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int): LiveOnlineUserHolder {
        return LiveOnlineUserHolder(LayoutInflater.from(context).inflate(R.layout.app_item_live_manage_user, parent, false))
    }
