...
class HighEndAuthEducationFragment : LazyFragment() {
    ...
    override fun getContentViewId(): Int {
        return R.layout.app_fragment_high_end_auth_education
    }
    ...
    private fun initRecycleViewAdapter() {
        mHighAuthAdapter = HighAuthAdapter(context, mList, "3")
        with(recycleView) {
    ...
    private fun getAuthInfo() {
        HttpHelper.getInstance().getHighAuthInfo("3", object : HttpCodeMsgListener {
    ...
    fun openPhotoWrapper() {// 拍照后照片的存放目录，改成你自己拍照后要存放照片的目录。如果不传递该参数的话就没有拍照功能
        var photoLayout = recycleView.findViewHolderForAdapterPosition(mLayoutPosition).itemView.findViewById(R.id.nine_layout_upload_photo) as BGASortableNinePhotoLayout
        ...
        val photoPickerIntent = BGAPhotoPickerActivity.IntentBuilder(context)
                .cameraFileDir(takePhotoDir) // photoLayout，改成你自己拍照后要存放照片的目录。如果不传递该参数的话则不开启图库里的拍照功能
                .maxChooseCount(photoLayout.maxItemCount - photoLayout.itemCount) // 图片选择张数的最大值
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            ...
            NormalConstant.REQUEST_CODE_2 -> {
                val photoLayout: BGASortableNinePhotoLayout = recycleView.findViewHolderForAdapterPosition(mLayoutPosition).itemView.findViewById(R.id.nine_layout_upload_photo)
                photoLayout.data = BGAPhotoPickerActivity.getSelectedPhotos(data)
            }
