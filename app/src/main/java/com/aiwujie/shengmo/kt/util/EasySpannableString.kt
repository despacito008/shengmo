 ...
 * @Package: com.aiwujie.shengmo.kt.util
 * @ClassName: EasySpannableString
 * @Author: xmf
 * @CreateDate: 2022/4/19 17:09
 * @Description:
 ...
 *
 */
class EasySpannableString(private val context: Context, text: CharSequence) : SpannableString(text) {
     ...
     */
    fun first(target: String): EasySpannableString {
        rangeList.clear()
     ...
     */
    fun last(target: String): EasySpannableString {
        rangeList.clear()
     ...
     */
    fun all(target: String): EasySpannableString {
        rangeList.clear()
     ...
     */
    fun range(from: Int, to: Int): EasySpannableString {
        rangeList.clear()
     ...
     */
    fun ranges(ranges: MutableList<Pair<Int, Int>>): EasySpannableString {
        rangeList.clear()
     ...
     */
    fun between(startText: String, endText: String): EasySpannableString {
        rangeList.clear()
     ...
     */
    fun size(dp: Int): EasySpannableString {
     ...
     */
    fun scaleSize(proportion: Int): EasySpannableString {
        for (range in rangeList) {
     ...
     * 给target字串设置样式（斜体）
     */
    fun italic(): EasySpannableString {
        for (range in rangeList) {
     ...
     * 给target字串设置样式（正常）
     */
    fun normal(): EasySpannableString {
        for (range in rangeList) {
     ...
     * 给target字串设置样式（粗斜体）
     */
    fun bold_italic(): EasySpannableString {
        for (range in rangeList) {
     ...
     */
    fun font(font: String): EasySpannableString {
        for (range in rangeList) {
     ...
     * 给target字串添加删除线
     */
    fun strikethrough(): EasySpannableString {
        for (range in rangeList) {
     ...
     * 给target字串添加下划线
     */
    fun underline(): EasySpannableString {
        for (range in rangeList) {
     ...
     */
    fun bullet(dp: Int, @ColorRes colorRes: Int?): EasySpannableString {
        for (range in rangeList) {
     ...
     */
    fun textColor(@ColorRes colorRes: Int): EasySpannableString {
     ...
     * 将target字串作为下标
     */
    fun subscript(): EasySpannableString {
        for (range in rangeList) {
     ...
     * 将target字串作为上标
     */
    fun superscript(): EasySpannableString {
        for (range in rangeList) {
     ...
     */
    fun onClick(textView: TextView, onTextClickListener: () -> Unit): EasySpannableString {
        for (range in rangeList) {
     ...
     * 给[textView]设置一个点击事件[onTextClickListener]
     */
    fun onClick(textView: TextView,color: Int,onTextClickListener: () -> Unit): EasySpannableString {
        for (range in rangeList) {
            val span = object : ClickableSpan() {
                override fun onClick(widget: View?) {
                    onTextClickListener.invoke()
