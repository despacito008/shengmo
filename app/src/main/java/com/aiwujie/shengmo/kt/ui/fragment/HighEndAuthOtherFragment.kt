...
class HighEndAuthOtherFragment : LazyFragment() {
    ...
    private fun initRecycleViewAdapter() {
        mHighAuthAdapter = HighAuthAdapter(context, mList, "5")
        with(recycleView) {
    ...
    private fun getAuthInfo() {
        HttpHelper.getInstance().getHighAuthInfo("5", object : HttpCodeMsgListener {
...
//                    mList[mLayoutPosition].imgList.addAll(BGAPhotoPickerActivity.getSelectedPhotos(data))
