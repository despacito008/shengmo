...
class HomeHighUserFollowAdapter(var context: Context, var list: List<HighUserBean>) : RecyclerView.Adapter<HomeHighUserFollowAdapter.HomeHighUserViewHolder>() {
    ...
    inner class HomeHighUserViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var tvFollowStatus: TextView = itemView.findViewById(R.id.tv_follow_status)
        ...
        fun display(position: Int) {
            list[position].run {
                ...
                if (is_follow == "1") {
                    tvFollowStatus.text = "取消关注"
                    tvFollowStatus.setTextColor(context.resources.getColor(R.color.lightGray))
                    tvFollowStatus.background = ContextCompat.getDrawable(context, R.drawable.bg_high_cancle_follow_status)
                } else {
                    tvFollowStatus.text = "    关注    "
                    tvFollowStatus.background = ContextCompat.getDrawable(context, R.drawable.bg_high_follow_status)
                    tvFollowStatus.setTextColor(context.resources.getColor(R.color.tongyong))
                }

                tvFollowStatus.setOnClickListener {
//                    onItemChildClick?.onChildClick(position,tvFollowStatus)
                    setFollow(position, top_id, is_follow, tvFollowStatus)
                }
    ...
    private fun setFollow(position: Int, topUid: String, followStatus: String, textView: TextView) {
        HttpHelper.getInstance().setHighUserFollow(topUid, object : HttpCodeMsgListener {
            override fun onSuccess(data: String?, msg: String?) {
                msg?.showToast()
                if (followStatus == "1") {
                    list[position].is_follow = "2"
                    textView.text = "    关注    "
                    textView.background = ContextCompat.getDrawable(context, R.drawable.bg_high_follow_status)
                    textView.setTextColor(context.resources.getColor(R.color.tongyong))
                } else {
                    list[position].is_follow = "1"
                    textView.text = "取消关注"
                    textView.setTextColor(context.resources.getColor(R.color.lightGray))
                    textView.background = ContextCompat.getDrawable(context, R.drawable.bg_high_cancle_follow_status)
                }
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int): HomeHighUserViewHolder {
        return HomeHighUserViewHolder(LayoutInflater.from(context).inflate(R.layout.app_item_high_end_auth_user_follow, parent, false))
    }
