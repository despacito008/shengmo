 ...
 * 直播排行榜 - 红包榜
 */
class LiveRedEnvelopesRankFragment : LazyFragment() {
    ...
    companion object {
        ...
        fun newInstance(aType: Int) = LiveRedEnvelopesRankFragment().apply {
    ...
    var page = 0;
    var adapter: LiveRedEnvelopesRankAdapter? = null
    lateinit var rankList: ArrayList<LiveRankBean.DataBean>
    private fun getRankingData() {
        HttpHelper.getInstance().getLiveRedBagRankList(type, page, object : HttpCodeMsgListener {
                    ...
                    liveRankBean?.data?.let {
                        when (page) {
                            0 -> {
                                ...
                                rankList.clear()
                                rankList.addAll(it)
                                adapter = LiveRedEnvelopesRankAdapter(activity, rankList, type)
