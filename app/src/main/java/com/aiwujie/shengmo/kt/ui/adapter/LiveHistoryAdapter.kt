...
class LiveHistoryAdapter(var context: Context,var historyList:List<LiveHistoryBean.DataBean.ListBean>,val fragment:Fragment): RecyclerView.Adapter<LiveHistoryAdapter.LiveHistoryHolder>() {
        ...
        fun display(index:Int) {
            historyList[index]?.let {
                if (TextUtils.isEmpty(it.live_poster)) {
                    //Glide.with(context).load(TUIKitLive.getLoginUserInfo().faceUrl).into(ivItemPoster)
                    Glide.with(fragment).load(R.drawable.ic_avatar).into(ivItemPoster)
                } else {
                    Glide.with(fragment).load(it.live_poster).into(ivItemPoster)
                }
