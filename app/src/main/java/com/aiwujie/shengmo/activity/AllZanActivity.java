...
import com.aiwujie.shengmo.adapter.AllZanAdapter;
...
public class AllZanActivity extends AppCompatActivity implements PullToRefreshBase.OnRefreshListener2, AdapterView.OnItemClickListener {
    ...
    private List<AllZanData.DataBean> zans = new ArrayList<>();
    private AllZanAdapter zanAdapter;
    /**
     * 判断是否继续刷新
     */
    ...
    private void getAllZan() {
        ...
        manager.post(HttpUrl.GetLaudedList, map, new IRequestCallback() {
            @Override
            public void onSuccess(final String response) {
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            AllZanData listData = new Gson().fromJson(response, AllZanData.class);
                            if (listData.getData().size() == 0) {
                                ...
                                if (page == 0) {
                                    ...
                                    zanAdapter = new AllZanAdapter(AllZanActivity.this, zans);
