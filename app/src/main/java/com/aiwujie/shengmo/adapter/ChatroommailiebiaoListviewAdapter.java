...
import com.aiwujie.shengmo.bean.ChatroomMaiListBean;
 ...
 */
public class ChatroommailiebiaoListviewAdapter extends BaseAdapter {
    private Context context;
    private List<ChatroomMaiListBean.DataBean> list;
    private LayoutInflater inflater;
    Handler handler = new Handler();

    public ChatroommailiebiaoListviewAdapter(Context context, List<ChatroomMaiListBean.DataBean> list) {
        super();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ChatroomMaiListBean.DataBean data = list.get(position);
      ...
      /*  if (data.getUserInfo().getOnlinestate() == 0) {
       ...
       // UserIdentityUtils.showIdentity(context,data.getHead_pic(),data.getUid(),data.getUserInfo().getIs_volunteer(),data.getUserInfo().getIs_admin(),data.getUserInfo().getSvipannual(),data.getUserInfo().getSvip(),data.getUserInfo().getVipannual(),data.getUserInfo().getVip(),holder.itemAtListviewVip);
        ...
        if (null !=data.getRule() &&"1".equals(data.getRule())){
            holder.itemAtListviewVip.setImageResource(R.mipmap.lanseren);
            holder.itemAtListviewVip.setVisibility(View.VISIBLE);
        }else {
            holder.itemAtListviewVip.setVisibility(View.GONE);
        }
