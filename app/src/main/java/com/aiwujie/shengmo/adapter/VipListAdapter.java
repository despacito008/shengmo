 ...
 */
public class VipListAdapter extends BaseAdapter implements View.OnClickListener {
    private Context context;
    private List<NoticeVipData.DataBean> list;
    private LayoutInflater inflater;
    private int[] vips = {R.drawable.user_vip, R.drawable.user_vip, R.drawable.user_vip, R.drawable.user_vip_year};
    private int[] svips = {R.drawable.user_svip, R.drawable.user_svip, R.drawable.user_svip, R.drawable.user_svip_year};
    public VipListAdapter(Context context, List<NoticeVipData.DataBean> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.item_listview_notice_vip, null);
        ...
        NoticeVipData.DataBean data = list.get(position);
        String vipContent = "";
        if(data.getState().equals("2")){
            //holder.itemListviewNoticeAutoLlbg.setBackgroundResource(R.mipmap.songlibeijing);
            holder.itemListviewNoticeVipIcon.setImageResource(vips[Integer.parseInt(data.getType()) - 1]);
            switch (data.getType()) {
                case "1":
                    vipContent = "1个月VIP";
                    break;
            ...
            //holder.itemListviewNoticeAutoLlbg.setBackgroundResource(R.mipmap.songlibeijingjin);
            holder.itemListviewNoticeVipIcon.setImageResource(svips[Integer.parseInt(data.getType()) - 1]);
            switch (data.getType()) {
                case "1":
                    vipContent = "1个月SVIP";
                    break;
