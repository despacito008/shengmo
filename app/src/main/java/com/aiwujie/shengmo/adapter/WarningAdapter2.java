...
public class WarningAdapter2 extends BaseAdapter {
    private Context context;
    private LayoutInflater inflater;
    private List<WarningListData.DataBean> list;
    private boolean isShowFlag;

    public WarningAdapter2(Context context, List<WarningListData.DataBean> list, boolean isShowFlag) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        WarningAdapter.ViewHolder holder;
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.item_listview_warning, null);
            holder = new WarningAdapter.ViewHolder(convertView);
            convertView.setTag(holder);
            //对于listview，注意添加这一行，即可在item上使用高度
            AutoUtils.autoSize(convertView);
        } else {
            holder = (WarningAdapter.ViewHolder) convertView.getTag();
        }
...
//            }
            if (data.getStatus().equals("0")) {
                holder.itemListviewWarningBanUser.setVisibility(View.VISIBLE);
                holder.tvItemBanAccount.setText("已禁账号-"+getBanTime(data.getAccount_blockingalong()));
            } else {
                holder.itemListviewWarningBanUser.setVisibility(View.GONE);
            }
            if (data.getDevicestatus().equals("0")) {
                holder.itemListviewWarningBanDevice.setVisibility(View.VISIBLE);
