 ...
 */
public class GzFsHyListviewAdapter extends BaseAdapter implements View.OnClickListener {
    ...
    public GzFsHyListviewAdapter(Context context, List<GzFsHyListviewData.DataBean> list, int who, String type) {
        super();
    ...
    public GzFsHyListviewAdapter(Context context, List<GzFsHyListviewData.DataBean> list, int who, String type,String name) {
        super();
        this.context = context;
        this.list = list;
        this.who = who;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.item_attetion_and_fans_listview, null);
        ...
        if (data.getUserInfo().getHead_pic().equals("") || data.getUserInfo().getHead_pic().equals(NetPic())) {//"http://59.110.28.150:888/"
            holder.ivUserIcon.setImageResource(R.mipmap.morentouxiang);
        } else {
            GlideImgManager.glideLoader(context, data.getUserInfo().getHead_pic(), R.mipmap.morentouxiang, R.mipmap.morentouxiang, holder.ivUserIcon, 0);
        }
        holder.tvLayoutUserNormalInfoAge.setText(data.getUserInfo().getAge());
        if (data.getUserInfo().getOnlinestate() == 0) {
            holder.ivUserOnline.setVisibility(View.GONE);
        } else {
            holder.ivUserOnline.setVisibility(View.VISIBLE);
        }
        //  判断显示标识，志愿者>官方管理>年svip>svip>年vip>vip
        UserIdentityUtils.showIdentity(context, data.getUserInfo().getHead_pic(), data.getUid(), data.getUserInfo().getIs_volunteer(), data.getUserInfo().getIs_admin(), data.getUserInfo().getSvipannual(), data.getUserInfo().getSvip(), data.getUserInfo().getVipannual(), data.getUserInfo().getVip(), data.getUserInfo().getBkvip(), data.getUserInfo().getBlvip(), holder.ivUserLevel);
     /*   if (data.getUserInfo().getIs_hand().equals("0")) {
        ...
        if (who == 0) {
            holder.tvCityya2.setVisibility(View.VISIBLE);
            if (data.getUserInfo().getLocation_city_switch().equals("1")) {
                holder.tvCityya2.setText("隐身");
            } else {
                if (data.getUserInfo().getCity().equals("") && data.getUserInfo().getProvince().equals("")) {
                    ...
                    if (data.getUserInfo().getProvince().equals(data.getUserInfo().getCity())) {
                        holder.tvCityya2.setText(data.getUserInfo().getCity());
                    } else {
                        holder.tvCityya2.setText(data.getUserInfo().getCity());
                    }
                }
            ...
            holder.tvCityya2.setVisibility(View.GONE);
            if (data.getUserInfo().getLocation_city_switch().equals("1")) {
        ...
        if (!data.getUserInfo().getCharm_val().equals("0")) {
            ...
            holder.tvLayoutUserNormalInfoCharm.setText(data.getUserInfo().getCharm_val());
        } else {
        ...
        if (!data.getUserInfo().getWealth_val().equals("0")) {
            ...
            holder.tvLayoutUserNormalInfoWealth.setText(data.getUserInfo().getWealth_val());
        } else {
        ...
        if (!TextUtil.isEmpty(data.getUserInfo().getAnchor_room_id()) && !"0".equals(data.getUserInfo().getAnchor_room_id())) {
            holder.ivUserState.setVisibility(View.VISIBLE);
            holder.lottieUserAvatarState.setVisibility(View.INVISIBLE);
            if ("0".equals(data.getUserInfo().getAnchor_is_live())) {
                Glide.with(context).load(R.drawable.ic_user_liver).into(holder.ivUserState);
            } else {
                ...
                holder.lottieUserAvatarState.loop(true);
                holder.lottieUserAvatarState.playAnimation();
            }
        } else {
            holder.ivUserState.setVisibility(View.INVISIBLE);
            holder.lottieUserAvatarState.setVisibility(View.INVISIBLE);
        }
