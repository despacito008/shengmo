...
public class UserGroupingFriendAdapter extends RecyclerView.Adapter<UserGroupingFriendAdapter.UserRelationShipHolder> {
    private Context context;
    private List<FenzuListData.DataBean> userList;
    private String keyWord;
    private boolean isSelf;

    public UserGroupingFriendAdapter(Context context, List<FenzuListData.DataBean> userList, String keyWord, boolean isSelf) {
    ...
    public UserRelationShipHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.app_item_user_friend_grouping, parent, false);
    ...
    class UserRelationShipHolder extends RecyclerView.ViewHolder {
        ...
        public void display(int index) {
            FenzuListData.DataBean user = userList.get(index);
            ...
            if ("1".equals(user.getLocation_city_switch())) {
                tvItemCity.setText("隐身");
            } else {
                if (TextUtil.isEmpty(user.getCity()) && TextUtil.isEmpty(user.getProvince())) {
                    tvItemCity.setText("(用户已隐藏位置)");
                } else {
                    if (user.getProvince().equals(user.getCity())) {
                        tvItemCity.setText(user.getCity());
                    } else {
                        tvItemCity.setText(user.getProvince() + " " + user.getCity());
                    }
                }
            }

            if (TextUtil.isEmpty(user.getLmarkname())) {
                tvLayoutUserNormalInfoCity.setText("");
            } else {
                tvLayoutUserNormalInfoCity.setText(user.getLmarkname());
            }
    ...
    private void followUser(int index) {
        boolean isFollow = userList.get(index).getState().equals("1") || userList.get(index).getState().equals("3");
        HttpHelper.getInstance().followUser(!isFollow, userList.get(index).getUid(), new HttpCodeMsgListener() {
            ...
            public void onSuccess(String data, String msg) {
                ...
                switch (Integer.parseInt(userList.get(index).getState())) {
                    case 0:
