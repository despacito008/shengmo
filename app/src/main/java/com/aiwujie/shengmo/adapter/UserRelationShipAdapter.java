...
public class UserRelationShipAdapter extends RecyclerView.Adapter<UserRelationShipAdapter.UserRelationShipHolder> {
    private Context context;
    private List<GzFsHyListviewData.DataBean> userList;
    private String keyWord;
    private boolean isSelf;

    public UserRelationShipAdapter(Context context, List<GzFsHyListviewData.DataBean> userList, String keyWord, boolean isSelf) {
    ...
    class UserRelationShipHolder extends RecyclerView.ViewHolder {
        ...
        public void display(int index) {
            ...
            if (isSelf) {
                ...
                ivItemUserRelationship.setVisibility(View.GONE);
            }
    ...
    private void followUser(int index) {
        boolean isFollow = userList.get(index).getFriend_state() == 1 || userList.get(index).getFriend_state() == 3;
        HttpHelper.getInstance().followUser(!isFollow, userList.get(index).getUid(), new HttpCodeMsgListener() {
            ...
            public void onSuccess(String data, String msg) {
                ...
                switch (userList.get(index).getFriend_state()) {
                    case 0:
                        userList.get(index).setFriend_state(1);
                        break;
                    case 1:
                        userList.get(index).setFriend_state(0);
                        break;
                    case 2:
                        userList.get(index).setFriend_state(3);
                        break;
                    case 3:
                        userList.get(index).setFriend_state(2);
                        break;
                }
