...
public class PublicScreenRedBagAdapter extends RecyclerView.Adapter<PublicScreenRedBagAdapter.PublicScreenGiftHolder> {
    Context context;
    List<NoticePresentData.DataBean> noticeList;
    Fragment fragment;

    public PublicScreenRedBagAdapter(Context context, List<NoticePresentData.DataBean> noticeList, Fragment fragment) {
    ...
    public PublicScreenGiftHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_listview_notice_red_bag, parent, false);
    ...
    class PublicScreenGiftHolder extends RecyclerView.ViewHolder {
        ...
        void display(int index) {
            ...
            if ("1".equals(data.getNum())) {
               itemListviewNoticePresentName.setText("红包" + " (" + data.getBeans() + "魔豆)");
            } else {
               itemListviewNoticePresentName.setText("红包" + "x" + data.getNum() + " (" + data.getBeans() + "魔豆)");
            }
