...
public class TRTCAVCallImpl implements ITRTCAVCall {
    ...
    public void processInvite(String inviteID, String inviter, String groupID, List<String> inviteeList, String data) {
        ...
        try {
            ...
            if (extraMap.containsKey(CallModel.SIGNALING_EXTRA_KEY_ONE_ON_ONE)) {
                Object tempValue =  extraMap.get(CallModel.SIGNALING_EXTRA_KEY_ONE_ON_ONE);
                if (tempValue instanceof Double) {
                    callModel.isOneOnOne = ((Double) extraMap.get(CallModel.SIGNALING_EXTRA_KEY_ONE_ON_ONE)).intValue();
                }
                if (tempValue instanceof Boolean) {
                    callModel.isOneOnOne = ((Boolean) extraMap.get(CallModel.SIGNALING_EXTRA_KEY_ONE_ON_ONE)) ? 1:0;
                }
            }
    ...
    public void handleDialing(CallModel callModel, String user) {
        ...
        //Log.d("link link","one ==" + callModel.isOneOnOne);
        if (callModel.isOneOnOne == 1) {
            onInvitedUserListParam = new ArrayList<>();
            onInvitedUserListParam.add("10086");
        }
        if (mTRTCInteralListenerManager != null) {
    ...
    private String sendModel(final String user, int action, CallModel model) {
        ...
        if (isLink) {
            customMap.put(CallModel.SIGNALING_EXTRA_KEY_ONE_ON_ONE, 1);
        }
        // signalling
