...
public class TXRoomService implements ITXRoomService {
    ...
    private   Pair<String, TXCallback>   mAnchorLinkMicReqPair;    // 主动发起方记录
    public void requestJoinAudience(String reason,String uid, int timeout, final TXCallback callback) {
        final String inviteId = V2TIMManager.getSignalingManager().invite(uid,  IMProtocol.getJoinReqJsonStr(reason), true, null,  timeout, null);
        mAnchorLinkMicReqPair = new Pair<>(inviteId, callback);
    }
    ...
    public void setIsEnterRoom(boolean mIsEnterRoom) { this.mIsEnterRoom = mIsEnterRoom; }
    ...
    private void onRecvC2COrGroupCustomMessage(final String groupId,final TXUserInfo txUserInfo, byte[] customData) {
        final ITXRoomServiceDelegate delegate  = mDelegate;
                ...
                switch (action) {
                    ...
                    case IMProtocol.Define.CODE_ROOM_CUSTOM_MSG:
                        ...
                        if (delegate != null && cusPair != null) {
                            delegate.onRoomRecvRoomCustomMsg(groupId, cusPair.first, cusPair.second, txUserInfo);
                        }
                        break;
    ...
    private void onRecvSignalResponse(final String inviteId, final String invitee, String data, boolean accept) {
        ...
        switch (action) {
            case IMProtocol.Define.CODE_RESPONSE_JOIN_ANCHOR:
                ...
                if (!invitee.equals(mOwnerIMInfo.userId)) { //不是主播发出的 那么就是收到了观众的拒绝/同意连麦请求
                    Pair<String, TXCallback> linkMicPair = mAnchorLinkMicReqPair;
                    if (linkMicPair != null) {
                        String requestId = linkMicPair.first;
                        TXCallback callback = linkMicPair.second;
                        callback.onCallback(accept ? 0 : -1,data);
                    }
                }
            ...
            case IMProtocol.Define.CODE_REQUEST_JOIN_ANCHOR:
               ...
               if (mCurrentRoomStatus == TRTCLiveRoomDef.ROOM_STATUS_PK) {
                ...
                if (delegate != null) {
                    ...
                    if (userInfo != null) { //直播间的观众发送的连麦邀请
                        delegate.onRoomRequestJoinAnchor(userInfo, reqReason);
                    } else {
                        if (inviter.equals( mOwnerIMInfo.userId)) { //直播间的主播发送的连麦邀请
                            TXUserInfo txUserInfo = new TXUserInfo();
                            txUserInfo.userId = mOwnerIMInfo.userId;
                            txUserInfo.userName = mOwnerIMInfo.name;
                            txUserInfo.avatarURL = "";
                            delegate.onRoomRequestJoinAnchor(txUserInfo, inviteId);
                        }
                    }
                }
                break;
    ...
    private class LiveRoomSimpleMsgListener extends V2TIMSimpleMsgListener {
        ...
        public void onRecvGroupCustomMessage(String msgID, String groupID, V2TIMGroupMemberInfo sender, byte[] customData) {
            ...
            txUserInfo.userName = sender.getNickName();
            txUserInfo.avatarURL = sender.getFaceUrl();
            onRecvC2COrGroupCustomMessage(groupID,txUserInfo, customData);
        }
    }
