...
public class ConversationProvider implements IConversationProvider {
        ...
        for (int i = 0; i < mDataSource.size(); i++) {
            if (mDataSource.get(i).getConversationId().equals(conversationID)) {
                if (mDataSource.remove(i) != null) {
                    Log.v("删除会话","deleteConversation--:"+conversationID);
                    updateAdapter();
                }
                return;
            }
    ...
    public void updateAdapter() {
        ...
        if (messageCustomListenerList != null && messageCustomListenerList.size() > 0) {
            for (OnMessageCustomListener listener: messageCustomListenerList) {
                Log.v("messageCustomListener","messageCustomListenerList");
                listener.doMessageRefresh(this);
            }
        }
    }
    ...
    interface OnMessageCustomListener {
        void doMessageRefresh(ConversationProvider mProvider);
    }

    public List<OnMessageCustomListener> messageCustomListenerList;

    public void addMessageCustomListener(OnMessageCustomListener onMessageCustomListener) {
        if (messageCustomListenerList == null) {
            messageCustomListenerList = new ArrayList<>();
        }
        if (!messageCustomListenerList.contains(onMessageCustomListener)) {
            messageCustomListenerList.add(onMessageCustomListener);
        }
    }
}
