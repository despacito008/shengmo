...
public class MessageLayout extends MessageLayoutUI {
    ...
    private void initPopActions(final MessageInfo msg) {
        ...
        if (msg.isSelf()) {
            ...
            if (onManagePopActionClickListener != null) {
                action = new PopMenuAction();
                action.setActionName(getContext().getString(R.string.revoke_action));
                action.setActionClickListener(new PopActionClickListener() {
                    @Override
                    public void onActionClick(int position, Object data) {
                        //mOnPopActionClickListener.onRevokeMessageClick(position, (MessageInfo) data);
                        onManagePopActionClickListener.onRevokeMessageClick(position, (MessageInfo) data);
                    }
                });
                actions.add(action);
            }
...
//        actions.add(action);
        ...
        //转发
        if (msg.getStatus() != MessageInfo.MSG_STATUS_SEND_FAIL && msg.getTimMessage().getElemType() != V2TIMMessage.V2TIM_ELEM_TYPE_CUSTOM) {
            action = new PopMenuAction();
    ...
    public void postSetAdapter(MessageListAdapter adapter) {
        mAdapter.setOnItemClickListener(new OnItemLongClickListener() {
            ...
            public void onUserIconLongClick(View view, int position, MessageInfo info) {
                if (mOnItemLongClickListener != null) {
                    mOnItemLongClickListener.onUserIconLongClick(view, position, info);
                }
            }
        });
    }
    ...
    public interface OnManagePopActionClickListener {
        void onRevokeMessageClick(int position, MessageInfo msg);
    }
    OnManagePopActionClickListener onManagePopActionClickListener;

    public void setOnManagePopActionClickListener(OnManagePopActionClickListener onManagePopActionClickListener) {
        this.onManagePopActionClickListener = onManagePopActionClickListener;
    }
}
