...
public class DragLayout extends RelativeLayout {
    ...
    protected void onFinishInflate() {
        ...
        targetView = findViewById(R.id.pv);
    }
    ...
    private class ViewDragCallback extends ViewDragHelper.Callback {
        ...
        public void onViewPositionChanged(@NonNull View changedView, int left, int top, int dx, int dy) {
            if (top > originPoint.y) {
                ...
                if ((top - originPoint.y) > getMeasuredHeight() / 10) {
                    callEvent = true;
                } else {
                    callEvent = false;
                }
            } else if (top < originPoint.y)  {
                float a = (float) (originPoint.y - top) / (float) (getMeasuredHeight() - originPoint.y);
                setBackgroundColor(changeAlpha(0xff000000, 1 - a));
                targetView.setScaleX(1 - a);
                targetView.setScaleY(1 - a);
                if ((originPoint.y - top) > getMeasuredHeight() / 10) {
                    callEvent = true;
                } else {
                    callEvent = false;
                }
