...
public class NineGridView extends ViewGroup {
    ...
    private int singleImageSize = 260;              // 单张图片时的最大大小,单位dp
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mImageInfo != null && mImageInfo.size() > 0) {
            ...
            } else if(mImageInfo.size() <= 4&&mImageInfo.size()!=3){
...
//            }
            Glide.with(getContext()).load(mImageInfo.get(i).thumbnailUrl).apply(new RequestOptions().transform(new MultiTransformation(new CenterCrop(),new RoundedCorners(20)))).into(childrenView);
