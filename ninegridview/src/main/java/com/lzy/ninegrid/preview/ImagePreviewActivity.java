...
import com.lzy.ninegrid.SaveImageUtils;

import java.util.List;

@SuppressWarnings("unchecked")
public class ImagePreviewActivity extends Activity implements ViewTreeObserver.OnPreDrawListener, OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //this.getWindow().getDecorView().setBackgroundColor(Color.TRANSPARENT);// 当然也可以在 theme 里设置
        setContentView(R.layout.nine_activity_preview);
        ...
        tv_pager.setText(String.format(getString(R.string.select), currentItem + 1, imageInfo.size()));
       // ViewCompat.setTransitionName(viewPager,"share_img");
    }
    ...
    public boolean onPreDraw() {
        ...
        addIntoListener(valueAnimator,view);
     ...
     * 进场动画过程监听
     */
    private void addIntoListener(ValueAnimator valueAnimator, final View view) {
            ...
            case 0:
                new Thread() {
                    ...
                    public void run() {
                        super.run();
                        try {
                            final Bitmap myBitmap = Glide.with(getApplicationContext())//上下文
                                    .asBitmap()//必须
                                    .load(imageInfo.get(currentItem).getBigImageUrl())//url
//                                    .centerCrop()
                                    .into(750, 750)
                                    .get();
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    SaveImageUtils.saveImageToGallerys(ImagePreviewActivity.this, myBitmap);
                                }
                            });
